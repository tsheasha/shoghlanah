...
def view_all_notifications(request):
    ...
    notifications_count = Notice.objects.filter(recipient=request.user.id).count()
    return render_to_response('Notifications/notifications.html',{'notifications':notifications, 'notifications_count':notifications_count}, RequestContext(request))
    
def view_notification(request, notification_id):
    """
    Marks the notification as seen and then redirects to its link
    """
    notice = Notice.objects.get(id=notification_id)
    notice.unseen = False
    notice.save()
