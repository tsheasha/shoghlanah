...
CONSUMER_KEY = getattr(settings, 'GOOGLE_ID', '')
CONSUMER_SECRET = getattr(settings, 'GOOGLE_SECRET', '')
LINKEDIN_PERMISSIONS = getattr(settings, 'GOOGLE_PERMISSIONS', '')
...
def google_login(request):
    ...
    params["redirect_uri"] = request.build_absolute_uri(reverse("google_login_done"))
...
def google_login_done(request):
    ...
    if not result:
        logging.debug("Google Backend: Couldn't authenticate user with Django, redirecting to Register page")
        return redirect(reverse('home'))

    if isinstance(result, UserProfile):
        if result.is_active:
            result.google_link = GoogleUserProfile.objects.get(user=result).url
            result.save()
            login(request, result)
            logging.debug("Google Backend: Successfully logged in with LinkedIn!")
            if 'next' in request.session:
                next = request.session['next']
                del request.session['next']
                request.session['Social'] = "google"
