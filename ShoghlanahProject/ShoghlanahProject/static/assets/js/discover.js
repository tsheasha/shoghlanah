...
function showfilters(){
    if($('.dock').is(":hidden")){
        $('.dock').show('slide', {direction: 'left'}, 1500);
        $('#filtericon').css({'border-radius':'35px','border-bottom-right-radius': '0px', 'border-top-right-radius': '0px'});
        $('#Ifilter').toggleClass('icon-filter  icon-remove');
        mainHtml = $("#stream").html();
    }
    else{
        $('.dock').hide('slide', {direction: 'left'}, 500,function(){
            if(selectedfilter !== ''){
                selectedfilter.attr("checked", false);
            }
            $('#filtericon').css({'border-radius':'35px','border-bottom-right-radius': '35px', 'border-top-right-radius': '35px'});
            $('#Ifilter').toggleClass('icon-filter  icon-remove');
        });
